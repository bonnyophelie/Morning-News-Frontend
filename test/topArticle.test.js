...
const topArticleData = {
    title : "Test topArticle",
    author : "John Doe",
  ...
  it('Display topArticle content', () => {
    render(
      <Provider store={store}>
        <PersistGate persistor={persistor}> 
          <Article {...topArticleData} />
        </PersistGate>
      </Provider>
    )
    const topArticleTitle = screen.getByText(topArticleData.title);
    expect(topArticleTitle).toBeTruthy();
  
    const topArticleAuthor = screen.getByText(topArticleData.author, {exact: false});
    expect(topArticleAuthor).toBeTruthy();
  
    const topArticleDescription = screen.getByText(topArticleData.description);
    expect(topArticleDescription).toBeTruthy();
  });
