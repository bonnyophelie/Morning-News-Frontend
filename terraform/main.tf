...
resource "aws_instance" "frontend_instance" {
  ...
  tags = {
    Name = var.aws_frontend_instance_name
  }
}
...
locals {
  machine_names = [for name in aws_instance.frontend_instance.*.tags.Name : name]
  machine_ip    = [for ip in aws_instance.frontend_instance.*.public_ip : ip]
}

# Create a file with the Machine Names and their associated Public IP adresses
resource "local_file" "file" {
  content  = <<EOT
%{for ip in aws_instance.frontend_instance.*.public_ip}
